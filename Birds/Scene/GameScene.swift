...
class GameScene: SKScene {
    ...
    var pinchRecogniser = UIPinchGestureRecognizer()
    ...
    func setupGestureRecognisers() {
        ...
        pinchRecogniser = UIPinchGestureRecognizer(target: self, action: #selector(pinch))
        view.addGestureRecognizer(pinchRecogniser)
    }
    ...
    override func didSimulatePhysics() {
        guard let phsyicsBody = bird.physicsBody else { return }
        if roundState == .flying && phsyicsBody.isResting {
            gameCamera.setConstraints(with: self, and: mapNode.frame, to: nil)
